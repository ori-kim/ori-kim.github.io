...
export const Toc = ({ body }: { body: string }) => {
  const { activeHeading } = useSelectedHeading();

  const tocHeadings = parseToc(body);
      ...
      {tocHeadings.map(({ text, level, slug }, index) => (
        <li
          ...
          className={cn(
            ...
            'text-zic-700',
            activeHeading.value === text && activeHeading.level === level && 'font-semibold text-black dark:text-white'
          )}
        >
          <span
...
function parseToc(source: string) {
  return source
    .split('\n')
